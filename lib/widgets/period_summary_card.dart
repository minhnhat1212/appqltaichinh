...
class PeriodSummaryCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final currencyFormat = NumberFormat.currency(locale: 'vi', symbol: '₫'); // Format tiền tệ VNĐ
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        gradient: LinearGradient(
          colors: [
            Theme.of(context).primaryColor.withOpacity(0.8), // Màu primary đậm hơn
            Theme.of(context).primaryColor.withOpacity(0.6), // Màu primary nhạt hơn (tạo gradient)
          ],
        ...
        boxShadow: [
          BoxShadow(
            color: Theme.of(context).primaryColor.withOpacity(0.3), // Shadow màu primary với độ trong suốt 30%
        ...
        children: [
          ...
          Padding(
            ...
            child: Row(
              ...
              children: [
                _buildPeriodButton(context, 'Ngày', 'day'), // Nút chọn theo ngày
                _buildPeriodButton(context, 'Tuần', 'week'), // Nút chọn theo tuần
                _buildPeriodButton(context, 'Tháng', 'month'), // Nút chọn theo tháng
          ...
          Padding(
            padding: const EdgeInsets.symmetric(horizontal: 24, vertical: 16), // Padding ngang 24px, dọc 16px
            child: Column(
              children: [
                _buildSummaryRow(
                  context,
                ...
                const Divider(color: Colors.white30), // Đường phân cách màu trắng nhạt
                const SizedBox(height: 12),
                _buildSummaryRow(
                  context,
                  ...
                  summary.net >= 0 ? Colors.green[300]! : Colors.red[300]!, // Màu xanh nếu dương, đỏ nếu âm
                  currencyFormat,
  ...
  Widget _buildPeriodButton(BuildContext context, String label, String period) {
    final isSelected = selectedPeriod == period; // Kiểm tra period có đang được chọn không

    return GestureDetector(
      onTap: () => onPeriodChanged(period), // Gọi callback khi nút được nhấn
      child: AnimatedContainer(
        duration: const Duration(milliseconds: 300), // Thời gian animation 300ms
        padding: const EdgeInsets.symmetric(horizontal: 20, vertical: 10), // Padding ngang 20px, dọc 10px
        decoration: BoxDecoration(
          color: isSelected ? Colors.white : Colors.white.withOpacity(0.2), // Nền trắng nếu được chọn, trắng nhạt nếu không
          borderRadius: BorderRadius.circular(20), // Bo góc 20px
          boxShadow: isSelected // Chỉ hiển thị shadow khi được chọn
              ? [
                  BoxShadow(
                    color: Colors.white.withOpacity(0.3), // Shadow trắng với độ trong suốt 30%
                    blurRadius: 8, // Độ mờ
        ...
        child: Text(
          ...
          style: TextStyle(
            color: isSelected ? Theme.of(context).primaryColor : Colors.white, // Màu primary nếu được chọn, trắng nếu không
            fontWeight: isSelected ? FontWeight.bold : FontWeight.normal, // Chữ đậm nếu được chọn
    ...
    return Row(
      children: [
        Container(
          ...
          decoration: BoxDecoration(
            color: Colors.white.withOpacity(0.2), // Nền trắng nhạt (độ trong suốt 20%)
            borderRadius: BorderRadius.circular(12), // Bo góc 12px
          ),
          child: Icon(icon, color: color, size: 24), // Icon với màu và kích thước tùy chỉnh
        ),
        ...
        Expanded(
          child: Text(
            ...
            style: TextStyle(
              color: Colors.white.withOpacity(0.9), // Màu trắng với độ trong suốt 90%
              fontSize: isNet ? 18 : 16, // Font lớn hơn nếu là số dư ròng
              fontWeight: isNet ? FontWeight.bold : FontWeight.normal, // Chữ đậm nếu là số dư ròng
            ),
          ),
        ),
        Text(
          currencyFormat.format(amount), // Format số tiền theo VNĐ
          style: TextStyle(
            color: Colors.white, // Màu trắng
            fontSize: isNet ? 20 : 18, // Font lớn hơn nếu là số dư ròng
